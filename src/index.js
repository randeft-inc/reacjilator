...
const isAlreadyPosted = (messages, translation) => {
  ...
  messages.forEach((messageInTheThread) => {
    if (
      !alreadyPosted &&
      messageInTheThread.subtype &&
      messageInTheThread.blocks[0].text.text === translation
    ) {
      alreadyPosted = true
    }
  })
...
const postMessage = async (
  ...
  if (message.text) {
    // Check if the message has translated
    ...
    blocks.push(
      ...
      },
      {
        ...
        elements: [
          {
            type: 'mrkdwn',
            text: `A translation of the original message to :${emoji}: _(${lang})_`,
          },
        ],
      }
    )
